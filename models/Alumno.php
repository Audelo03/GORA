...
class Alumno {
    ...
    public function update() {
        $sql = "UPDATE " . $this->table . " 
                SET matricula = :matricula, nombre = :nombre, apellido_paterno = :apellido_paterno, apellido_materno = :apellido_materno,
                    estatus = :estatus, tutores_id_tutor = :tutor, carreras_id_carrera = :carrera, grupos_id_grupo = :grupo
                WHERE id_alumno = :id";
        $stmt = $this->conn->prepare($sql);
    ...
    public function getByTutorId($tutor_id) {
        $sql = "SELECT * FROM " . $this->table . " WHERE tutores_id_tutor = :tutor_id";
        $stmt = $this->conn->prepare($sql);     
